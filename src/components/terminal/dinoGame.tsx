...
const DinoGame = ({ quitGame }: Props) => {
  ...
  const getRandomSpawnInterval = () =>
    Math.floor(Math.random() * (MAX_OBSTACLE_SPAWN_INTERVAL - MIN_OBSTACLE_SPAWN_INTERVAL + 1)) + MIN_OBSTACLE_SPAWN_INTERVAL;
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      setObstacles((prev) => {
        ...
        // Spawn new obstacles
        if (now - lastObstacleSpawnRef.current >= nextObstacleSpawnRef.current) {
  ...
  return (
    <div className="relative w-full h-full bg-black border-4 border-green-500 font-mono text-green-500 overflow-hidden">
      ...
      {gameOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-75">
          <div className="text-center">
            ...
            <p>Score: {score}</p>
            <button
              className="mt-2 px-4 py-2 bg-green-500 text-black hover:text-white"
              onClick={restartGame}
            >
              Restart
            </button>
      ...
      <div
        className="absolute bottom-0 w-full h-2 bg-green-500"
        style={{ zIndex: 0 }}
      ></div>

      <div
        className="absolute bottom-0 w-10 h-10 bg-green-500 transition-transform duration-100"
        style={{
          left: `${DINO_X}px`,
      ...
      {obstacles.map((obs) => (
        <div
          key={obs.id}
          className="absolute bottom-0 w-5 h-10 bg-green-500"
